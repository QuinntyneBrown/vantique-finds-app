export class Router extends HTMLElement {
    ...
    _onChanged() {
        ...
        const route = routes.find(r => (r as any).test(path));
        const data = (route as any).exec(path);
    ...
    _addRoutes() {
        let views = Array.from(document.querySelectorAll('vf-view'));
        views.forEach(view => {
            if (!(view as any).route)
                return;

            this.addRoute(new RegExp((view as any).route, 'i'), view);
        }, this);
    }
    ...
    private _routes: any;
    private _newView: any;
    private _isTransitioningBetweenViews: any;
